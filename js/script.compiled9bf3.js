...
function relativeTime(date, period, short) {
  if (!period) period = relativeTimePeriods;
  if (!(date instanceof Date)) date = new Date(date * 1000);
  var seconds = (new Date() - date) / 1000;
  var _iterator = _createForOfIteratorHelper(period),
    _step;
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done; ) {
      ...
      if (seconds >= secondsPer) {
        ...
        if (short) return "".concat(amount).concat(name);
        return ""
          .concat(amount, " ")
          .concat(name)
          .concat(amount && amount !== 1 ? "s" : "", " ago");
      }
    }
  } catch (err) {
    _iterator.e(err);
  ...
  fetch("https://dimden.dev/services/nowplaying", {
    ...
    .then(function (i) {
      if (i["@attr"] && i["@attr"].nowplaying) {
        ...
        document.getElementById("currentlylistening").innerText =
          "Current song:";
        document.getElementById("currentlylistening").textContent =
          "Current song:";
        document.getElementById("status").innerText = "Online!";
        document.getElementById("status").textContent = "Online!";
        document.getElementById("status").style.color = "lime";
        document.getElementById("status").style.textShadow =
          "rgb(4 255 214 / 0%) 0px 0px 10px, rgb(0 255 236 / 17%) 0px 0px 20px, rgb(2 230 0 / 17%) 0px 0px 30px, rgb(0 230 178 / 0%) 0px 0px 40px, rgb(0 230 27 / 24%) 0px 0px 50px, rgb(0 230 178 / 0%) 0px 0px 60px, rgb(0 230 122 / 0%) 0px 0px 70px";
        ...
        if (i.date && i.date.uts) {
          document.getElementById("lastplayed").innerText = "Played ".concat(
            relativeTime(new Date(+(i.date.uts + "000")))
          );
          document.getElementById("lastplayed").textContent = "Played ".concat(
            relativeTime(new Date(+(i.date.uts + "000")))
          );
        }
      }
      ...
      if (i.name) {
        ...
        if (i.name.length > 11 && songname.tagName === "SPAN")
          songname.outerHTML = songname.outerHTML.replace(
            /<span(.*?)/g,
            "<marquee$1"
          );
...
function checkStatus() {
  fetch("https://dimden.dev/services/slave/status/")
    .then(function (i) {
      return i.json();
    })
    .then(function (i) {
      if (i.dimden === "online" || playing) {
        document.getElementById("message").placeholder = "Enter message - please be nice!";
        ...
        document.getElementById("message").placeholder = "Chat is currently offline";
        document.getElementById("message").disabled = true;
        document.getElementById("send").disabled = true;
        document.getElementById("status").innerText = "Offline!";
        document.getElementById("status").style.color = "red";
        document.getElementById("status").style.textShadow =
          "0 0 10px #04ffd518, 0 0 20px #00ffee13, 0 0 30px #e600002f, 0 0 40px #00e6b200, 0 0 50px #e600003d, 0 0 60px #e6000029, 0 0 70px #e6000057";
      }
      ...
      if (i.atabook === "online") {
        document.getElementById("atabook-status").innerText = " OK ";
        document.getElementById("atabook-status").textContent = " OK ";
        document.getElementById("atabook-status").className = "status-ok";
      } else {
        document.getElementById("atabook-status").innerText = "FAIL";
        document.getElementById("atabook-status").textContent = "FAIL";
        document.getElementById("atabook-status").className = "status-fail";
      }
      if (i.lune === "online") {
...
// last tweet
function getTweet() {
  fetch("https://dimden.dev/services/lasttweet")
    .then(function (i) {
      return i.json();
    })
    .then(function (t) {
      ...
      document.getElementById("tweet-date").innerText = relativeTime(
        t.date,
        relativeTimePeriodsShort,
        true
      );
      document.getElementById("tweet-date").textContent = relativeTime(
        t.date,
        relativeTimePeriodsShort,
        true
      );
  ...
  try {
    for (_iterator2.s(); !(_step2 = _iterator2.n()).done; ) {
      ...
      if (Date.now() - _c.lastMove > 60000) {
        if (!_c.cursor.hidden) _c.cursor.hidden = true;
      } else {
        if (_c.cursor.hidden) _c.cursor.hidden = false;
      }
    }
  } catch (err) {
    _iterator2.e(err);
...
function connectChat() {
  ...
  client.onmessage = function (msg) {
    if (typeof msg.data !== "string") {
      ...
      if (x === 0 && y === 0) _c2.cursor.hidden = true;
      else _c2.cursor.hidden = false;
      return;
    }
    ...
    if (msg.data.startsWith("Joined: ")) {
      var _msg$data$split$1$spl = msg.data
          .split(": ")[1]
          .split(",")
          .map(Number),
      ...
      _cursor.hidden = true;
      if (_x2 === 0 && _y === 0) _cursor.hidden = true;
      else _cursor.hidden = false;
    ...
    text.innerHTML = text.innerHTML.replace(
      /(?:&lt;|<):([A-Z-a-z\-_~0-9]+?):([0-9]+)(?:&gt;|>)/g,
      '<img class="emote" title="$1" src="https://cdn.discordapp.com/emojis/$2.png?v=1">'
    );
    text.innerHTML = text.innerHTML.replace(
      /(?:&lt;|<)a:([A-Z-a-z\-_~0-9]+?):([0-9]+)(?:&gt;|>)/g,
      '<img class="emote" title="$1" src="https://cdn.discordapp.com/emojis/$2.gif?v=1">'
    );
    text.className = "chat-msg ".concat(
      msg.data.startsWith("<LOCAL>") ? "chat-msg-local" : ""
    );
    ...
    try {
      date.title = "".concat(
        new Date(+msg.data.split(" | ").slice(-1)).toLocaleTimeString([], {
    ...
    setTimeout(function () {
      if (
        messages.scrollHeight - messages.scrollTop < 300 ||
        Date.now() - chatJoin < 5000
      )
  ...
  client.onclose = function () {
    ...
    var _iterator3 = _createForOfIteratorHelper(cursors),
      _step3;
    try {
...
document.getElementById("send").onclick = function () {
  ...
  if (msg.startsWith("/nick ")) {
    ...
    if (Date.now() - lastMessageSentDate < 1000) return;
...
// duplicate buttons couple of times to remove spaces
if (!navigator.userAgent.includes("PaleMoon")) {
  // stupid ass pale moon doesnt support marquee
  ...
  Array.from(siteButtons.children).forEach(function (e) {
    ...
    if (newButton.href === "https://melankorin.net/") {
      newButton.children[0].src =
        "https://melankorin.net/assets/img/buttons/button-2.gif";
    } else if (newButton.href === "https://dokodemo.neocities.org/") {
      newButton.children[0].src =
        "https://dokode.moe/media/mine/88x31/dokodemobutton2.gif";
...
muterain.addEventListener("change", function () {
  if (muterain.checked) {
    ...
    rainBgm.pause();
    if(window.snowBgm) snowBgm.pause();
    if (thunderBgm) thunderBgm.pause();
  } else {
    delete localStorage.muterain;
    rainBgm.play();
    if(window.snowBgm) snowBgm.play();
    if (thunderBgm) thunderBgm.play();
  }
});
...
// kanji stats
// fetch('https://kanji.dimden.dev/stats.json').then(function (r) {
//   return r.json();
// }).then(function (stats) {
//   var se = document.getElementById('kanji-stats');
//   se.innerHTML = "Kanji: ".concat(stats.learned_kanji, " \u2022 Vocab: ").concat(stats.learned_vocab);
// })["catch"](function (e) {
//   console.error(e);
// });
...
function updateMood() {
  fetch("https://mood.dimden.dev/services/mood/get")
    .then(function (r) {
      return r.json();
    })
    .then(function (m) {
      var info = getMoodInfo(m.pleasantness, m.energy);
      // mood.innerText = info[0];
      // mood.textContent = info[0];
      mood.innerText =
        (m.pleasantness > 0 ? "+" : "") +
        m.pleasantness.toFixed(2) +
        " / " +
        (m.energy > 0 ? "+" : "") +
        m.energy.toFixed(2);
      mood.textContent =
      ...
      document.getElementById("mood-emoji").src =
        "https://mood.dimden.dev/fbemoji/" + info[2] + ".png";
